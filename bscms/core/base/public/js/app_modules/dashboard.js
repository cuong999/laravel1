        ...
        success: function (res) {
            ...
            if (!res.error) {
                ...
                if (el.find('.scroller').length != 0) {
                    Bytesoft.callScroll(el.find('.scroller'));
                }
    ...
    Sortable.create(el, {
        ...
        onEnd: function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (!res.error) {
                        Bytesoft.showNotice('success', res.message, Bytesoft.languages.notices_msg.success);
                    } else {
                        Bytesoft.showNotice('error', res.message, Bytesoft.languages.notices_msg.error);
                    }
                },
                error: function (data) {
...
BDashboard.init = function () {
    ...
    list_widgets.on('change', '.number_record .numb', function (e) {
        ...
        if (!isNaN(paginate)) {
            ...
            Bytesoft.showNotice('error', 'Please input a number!', Bytesoft.languages.notices_msg.error)
        }
