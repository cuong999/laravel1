class Bytesoft {
    constructor() {
        ...
        Bytesoft.initResources();
        Bytesoft.handleCounterUp();
        Bytesoft.initMediaIntegrate();
    }
    ...
    static showNotice(messageType, message) {
        ...
        switch (messageType) {
            case 'error':
                messageHeader = BytesoftVariables.languages.notices_msg.error;
                break;
            case 'success':
                messageHeader = BytesoftVariables.languages.notices_msg.success;
                break;
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Bytesoft.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Bytesoft.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Bytesoft.showNotice('error', data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Bytesoft.showNotice('error', item);
                        });
                    });
                }
            } else {
                Bytesoft.showNotice('error', data.statusText);
            }
        }
    }
    ...
    static handleValidationError(errors) {
        ...
        Bytesoft.showNotice('error', message);
    }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + BytesoftVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initResources() {
        ...
        Bytesoft.initDatePicker('.datepicker');
        Bytesoft.callScroll($('.list-item-checkbox'));
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', (event) => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Bytesoft.getViewPort().height -
    ...
    static initCodeEditor(id) {
        ...
        $('.handle-tool-drag').mousedown((event) => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Bytesoft.onDragTool);
            $(window).on('mouseup', Bytesoft.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Bytesoft.onDragTool);
        $(window).off('mouseup', Bytesoft.onReleaseTool);
    }
}
