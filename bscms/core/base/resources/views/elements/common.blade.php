    ...
    @if (session()->has('success_msg') || session()->has('error_msg') || (isset($errors) && $errors->count() > 0) || isset($error_msg))
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                Bytesoft.showNotice('success', '{{ session('success_msg') }}');
                @endif
                @if (session()->has('error_msg'))
                Bytesoft.showNotice('error', '{{ session('error_msg') }}');
                @endif
                @if (isset($error_msg))
                Bytesoft.showNotice('error', '{{ $error_msg }}');
                @endif
                @if (isset($errors))
                @foreach ($errors->all() as $error)
                Bytesoft.showNotice('error', '{{ $error }}');
                @endforeach
                @endif
            });
        </script>
